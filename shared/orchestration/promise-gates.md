...
1. **Walkthrough** (all tiers) — a fresh agent uses the product the way the promise's user would, and reports in plain English. See `../verification/walkthrough-verification.md`. At T0–T1 this may be the entire gate.
...
- the intent auditor has not returned `misaligned` (a `misaligned` verdict routes through the authority rules in `../review/intent-audit.md` — it is not one more finding to remediate around).
