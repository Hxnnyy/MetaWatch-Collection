...
- T2: only on genuine plan-level disagreement, architecture risk, or broad multi-system scope.
- T3: default before the PRD.
- T0–T1: never.

## Roles

- **Members** — `models.council`. Independent reviews, written blind to each other, each with an assigned lens. One member (strongest model, `routing.intentAuditor` alias by default) carries the pragmatist brief: the smallest implementation that keeps every promise true.
- **Chair** — `models.councilChair` (default `frontier-oss`), from a lab unrepresented among members. Dispositions every finding in one pass; does not vote.

## The Round

1. Freeze the proposal version; distribute intent contract + proposal + packet.
