...
1. **Walkthrough** (T1+) — a fresh agent uses the product the way the promise's user would, and reports in plain English. See `walkthrough-verification.md`. At T1 this is normally the entire gate.
...
- the intent auditor (T2+) is `aligned` when required (a `drifting` or `misaligned` verdict routes through the authority rules in `intent-audit.md` — it is not one more finding to remediate around).
