...
- **`promises` is mandatory and non-empty.** An item that serves no promise does not get created — that is the coverage audit's contract (`intent-audit.md`). Enabling work (build plumbing, shared fixtures) cites the promise it unblocks.
