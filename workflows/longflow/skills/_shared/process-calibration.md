...
None of this is fixed by telling the model to "be pragmatic" — under pressure, exhortation loses to training. It is fixed structurally: an explicit tier decision at kickoff, per-item rigour classes, an asymmetric rule that makes *adding* ceremony the move that needs permission, and independent fresh-context audits with authority to descope (see `intent-audit.md`).
...
- `intent-audit.md` — the independent check on calibration decisions.
