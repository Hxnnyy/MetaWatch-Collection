...
3. **Delivery is the standard pipeline.** Slice per `prd-to-issues` (ledger items, promise citations, rigour classes, sizes, proportionate checks, coverage audit), execute per `issues-execution` (continuous by default, promise gates keyed to the dimension thresholds). Do not re-implement either loop here.
...
**Phase gate**: "Ready to slice and fix? Delivery runs continuously by default — say `interactive mode` for gating." — audit + triage is a legitimate deliverable.
...
3. Closeout comes from `issues-execution` as normal: full test suite, predicate roll-up, final intent audit, plain-English report (dimensions improved and by how much, gaps left with explicit justification), retro to `RUNS.md`.
